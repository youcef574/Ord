    ...
    constructor() {
        ...
        this.translations = {
            fr: {
                ...
                // Enhanced Status - 12 States
                statusNouveau: 'Nouveau',
                statusEnCours: 'En cours',
                statusEnPreparation: 'En préparation',
                statusExpedie: 'Expédié',
                statusTermine: 'Terminé',
                statusAnnule: 'Annulé',
                statusRetour: 'Retour',
                statusAttenteEchange: 'En attente d\'échange',
                statusEchangeExpedie: 'Échange expédié',
                statusEchangeTermine: 'Échange terminé',
                statusRemboursement: 'Remboursement en cours',
                statusLitige: 'Litige ouvert'
            },
            en: {
                // Header
                dashboard: 'Dashboard',
              ...
              //  couleur: 'Color',
                ...
                statusEnPreparation: 'In preparation',
                statusExpedie: 'Shipped',
                statusTermine: 'Completed',
                statusAnnule: 'Cancelled',
                statusRetour: 'Returned',
                statusAttenteEchange: 'Awaiting exchange',
                statusEchangeExpedie: 'Exchange shipped',
                statusEchangeTermine: 'Exchange completed',
                statusRemboursement: 'Refund in progress',
                statusLitige: 'Dispute opened'
            }
        };
...
class OrdersManager {
    ...
    getStatusConfig() {
        return {
            ...
            'en-preparation': { 
                icon: 'fas fa-cogs', 
                key: 'statusEnPreparation',
                color: 'en-preparation' 
            },
            'expedie': { 
                icon: 'fas fa-truck', 
                key: 'statusExpedie',
            ...
            'attente-echange': { 
                ...
                key: 'statusAttenteEchange',
                color: 'attente-echange' 
            },
            'echange-expedie': { 
                icon: 'fas fa-exchange-alt', 
                key: 'statusEchangeExpedie',
                color: 'echange-expedie' 
            },
            'echange-termine': { 
                icon: 'fas fa-check-double', 
                key: 'statusEchangeTermine',
                color: 'echange-termine' 
            },
            'remboursement': { 
                icon: 'fas fa-money-bill-wave', 
                key: 'statusRemboursement',
                color: 'remboursement' 
            },
            'litige': { 
    ...
    renderStatusDropdown(orderId, currentStatus) {
        ...
        const currentConfig = statusConfig[currentStatus] || statusConfig['nouveau'];
        
        // Generate dropdown items
        const dropdownItems = Object.entries(statusConfig).map(([status, config]) => {
            const isActive = status === currentStatus ? 'active' : '';
            return `
                <div class="status-dropdown-item ${isActive}" data-status="${status}">
                    <div class="status-icon ${config.color}"></div>
                    <span>${window.langManager.t(config.key)}</span>
                </div>
            `;
        }).join('');
        ...
        return `
            <div class="status-dropdown" data-order-id="${orderId}">
                <div class="status-display ${currentConfig.color}">
                    <i class="${currentConfig.icon}"></i>
                    <span>${window.langManager.t(currentConfig.key)}</span>
                </div>
    ...
    updateStatusFilter() {
        ...
        Object.entries(statusConfig).forEach(([status, config]) => {
            ...
            option.value = status;
            option.textContent = window.langManager.t(config.key);
            option.setAttribute('data-key', config.key);
    ...
    updateStats() {
        const stats = {
            new: this.orders.filter(o => o.status === 'nouveau').length,
            processing: this.orders.filter(o => ['en-cours', 'en-preparation'].includes(o.status)).length,
            shipped: this.orders.filter(o => ['expedie', 'echange-expedie'].includes(o.status)).length,
            completed: this.orders.filter(o => ['termine', 'echange-termine'].includes(o.status)).length
        };
